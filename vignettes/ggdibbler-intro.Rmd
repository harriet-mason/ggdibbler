...
Here is an example with ggraph and uncertain edges represented by a sample of simulated edges. You need to apply `sample_expand` before you convert the data to a graph, because `ggraph` is all totalitarian about what it will keep in its graph data set, and it doesn't allow distributions. 
...
# IDEAL case would be if you could add some transparency and jitter
# but jittering doesnt jitter the line... it does whatever this is
# which kind of works, makes it look a little sketchy or something
...
# you can mess around with the alpha/linewidth to get something you are happy
# with

# The geom_edge_density makes the next most sense...
# but the example deadass gives me an error
# oi @thomas why am I being made to suffer like this
# ggraph(graph_sample, layout = 'fr', weights = weight) + 
#   geom_edge_density() + 
#   geom_node_point(size=10)

# There are a few remaining options that kind of capture the sample element

# uncertainty indicated by transparency
ggraph(graph_sample, layout = 'fr', weights = weight) + 
  geom_edge_link(aes(group=drawID), alpha=0.01,
                 linewidth=2) + 
  geom_node_point()
...
ggraph(graph_sample, layout = 'fr', weights = weight) + 
  geom_edge_parallel0(aes(group=drawID),
                      sep = unit(0.05, "mm"), alpha=0.5, linewidth=0.05) + 
  geom_node_point(size=10)

# kind of like sample? tbh I have no idea what this edge is doing
ggraph(graph_sample, weights = weight) + 
  geom_edge_bundle_force(aes(group=drawID), n=300, threshold = 5, alpha=0.2) + 
  geom_node_point(size=5)
```
