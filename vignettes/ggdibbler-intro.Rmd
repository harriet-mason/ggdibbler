...
forecast <- as_tsibble(sunspot.year) |> 
  model(ARIMA(value)) |> 
  forecast(h = "10 years") |>
  mutate(increment = value - lag(.mean))
forecast$increment[1] <- forecast$value[1]

ggplot(forecast, aes(colour = increment)) +
  geom_line_sample(aes(x = index, 
                       y = stats::lag(after_stat(colour), k=10) + after_stat(colour)),
                       times=100)
...
library(dplyr)
library(conmat)
# This is my version of the package to use the seed argument
library(ggdibbler)
