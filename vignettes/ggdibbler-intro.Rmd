...
### Random variables as "units" and `distributional`
...
Before I go into distributional, I wanted to explain why you are forced to pass a distribution instead of an estimate and standard error which is common among other uncertainty visualisation packages.
...
### Uncertainty visualisation as random variable
...
The final element of the `ggdibbler` approach is the idea that the uncertainty *should* interfere with your ability to read the plot, for better or worse. To some people this seems obvious, but we have found that this concept is not easy for everyone to grasp. 

For invalid signals to become invisible, you *must* give your noise the ability to interefere with your reading of the plot. This is the opposite approach we typically have when mapping variables. Usually, when plotting two variables, we want the second variable *not* to interfere. If we are plotting height and age, we don't want values that are particularly high on weight to have an impossible to read weight. However, that desire for visual independence is born from variable independence. This assumption no longer holds in the case of uncertainty and estimate. The variables aren't even different variables, they are two parameters of the same distribution. 
...
These three components each answer a related question about how we want to quantify our data.
...
3. Position nesting: How are you making all the values values from the distribution visible (e.g. layering and setting `alpha < 1`, using a `position_dodge`, etc)? 

`ggdibbler` has zero unique features that were not available already in ggplot. The entire package is made by nesting the already existing grammar. There is exactly *one* new feature we implemented that is not a variation of what was already in `ggplot` and that is the `position_subdivide` based on the Vizumap's pixel map, and the only reason we had to implement that was because `position_dodge` doesn't work on SF objects or polygons (although I think they should). There are three nested components implemented in the package.

The order of the nesting depends on which pat
1. A nested statistic that scales the distribution while keeping it as a random variable
2. A nested statistic that represent your distribution which is nested inside the statistic you want to calculate
3. The position adjustment you want to use for your distribution nested inside the position adjutment used on the ggplot version of the graphic.

Even with this list,`ggdibbler` does not implement this full nested grammar of graphics. The package would be greatly expanded by a full nested positioning system and having a quantile variation of all the `stat_*_sample` functions, but these features will take time. The nested statistic is neat, but I would really reccomend playing around with the nested positions.
...
## Examples in `ggdibbler`
### Time series output
Sometiems you will be woring with a package that directly spits out distributional objects, such as `fable`. In this case, you can literally just directly visualise the uncertainty with zero preprocessing
```{r}
...
The more R programs that spit out a distribution directly, the easier it will be for you to visualise uncertainty. This is not the case usually, but when it works the workflow is so seamless its hard to know you are even visualising uncertainty.
...
### A contact map and doubling up on random variables
This example is from the `conmat` R package readme. The point of this example is to show that two observations can use the same random variables, they just need to split up the dats. 
