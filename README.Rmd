...
<!-- badges: start -->
[![Codecov test coverage](https://codecov.io/gh/harriet-mason/ggdibbler/graph/badge.svg)](https://app.codecov.io/gh/harriet-mason/ggdibbler)
<!-- badges: end -->
...
The eventual goal of `ggdibbler` is to allows any distribution object to be passed to any aesthetic in a `ggplot2` geom (so long as I have gotten to that geom). The status of this goal is regularly updated in the [All Geometries - Status List](https://github.com/harriet-mason/ggdibbler/issues/28) issue. 
...
## Examples
For a proper discussion of how to use `distributional` and `ggdibbler` to incorporate uncertainty into your plots, feel free to check out the vignette. For the short version, you can just replace the `geom_*` with `geom_*_sample` and replace a variable that was once values with distributions.
