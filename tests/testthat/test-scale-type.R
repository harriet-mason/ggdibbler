...
test_that("Sample Distributions", {
  ...
  expect_equal(scale_type.distribution(dist1), c("distribution","continuous"))
  
  # Categorical Sample
  dist2 <- dist_sample(list(c("cat", "dog", "hen", "hen", "cat")))
  expect_equal(scale_type.distribution(dist2), c("distribution","discrete"))
})
...
test_that("Standard Distributions", {
  ...
  expect_equal(scale_type.distribution(dist2), c("distribution","continuous"))
  
  # Uniform
  dist3 <- dist_uniform(min = c(3, -2), max = c(5, 4))
  expect_equal(scale_type.distribution(dist3), c("distribution","continuous"))
})