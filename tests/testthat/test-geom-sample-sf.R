...
test_that("geom_sample_sf tests", {
  set.seed(1)
  
  # basic geom_sf check
  p1 <- basic_data |>  
    ggplot2::ggplot() + 
    geom_sample_sf(ggplot2::aes(geometry = county_geometry, fill=temp_dist))
  ...
  p3 <- basic_data |>  
    ggplot2::ggplot() + 
    geom_sample_sf(ggplot2::aes(geometry = county_geometry, fill=temp_dist), times=12)
